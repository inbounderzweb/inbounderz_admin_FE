...
function App() {
  const { activePage, isAuthenticated, user, isAuthLoading, checkAuth, setActivePage } = useAppStore();
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-2 border-white/10 border-t-[#f5c842] rounded-full animate-spin" />
          <p className="text-white/40 text-[12px] font-medium tracking-widest uppercase">Verifying Session</p>
        </div>
      </div>
    );
  }
  ...
  if (user?.role !== 'admin') {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white p-6 text-center animate-in fade-in duration-700">
